...
class FlatToRawCompilerArgumentsBucketConverter(override val classLoader: ClassLoader? = null) :
    ...
    override fun convert(from: FlatCompilerArgumentsBucket): RawCompilerArgumentsBucket = mutableListOf<String>().apply {
        ...
        obtainArgumentAnnotation(classLoader, COMMON_ARGUMENTS_CLASS, "pluginClasspaths")?.also {
            from.pluginClasspaths.ifNotEmpty { add("${it.value}=${joinToString(it.delimiter)}") }
        }
        (obtainArgumentAnnotation(classLoader, JVM_ARGUMENTS_CLASS, "friendPaths")
            ?: obtainArgumentAnnotation(classLoader, METADATA_ARGUMENTS_CLASS, "friendPaths"))?.also {
...
class RawToFlatCompilerArgumentsBucketConverter(override val classLoader: ClassLoader? = null) :
    ...
    override fun convert(from: RawCompilerArgumentsBucket): FlatCompilerArgumentsBucket {
        val pluginClasspathsAnnotation = obtainArgumentAnnotation(classLoader, COMMON_ARGUMENTS_CLASS, "pluginClasspaths")
        val existingPluginClasspaths = pluginClasspathsAnnotation?.let {
            from.firstOrNull { it1 -> it1.startsWith(it.value) }
        }

        val friendPathsAnnotation = (obtainArgumentAnnotation(classLoader, JVM_ARGUMENTS_CLASS, "friendPaths")
            ?: obtainArgumentAnnotation(classLoader, METADATA_ARGUMENTS_CLASS, "friendPaths"))
        val existingFriendPaths = friendPathsAnnotation?.let {
            from.firstOrNull { it1 -> it1.startsWith(it.value) }
        }

        val existingClasspaths = (obtainArgumentAnnotation(classLoader, JVM_ARGUMENTS_CLASS, "classpath")
            ?: obtainArgumentAnnotation(classLoader, METADATA_ARGUMENTS_CLASS, "classpath"))?.let {
        ...
        val pluginClasspaths = pluginClasspathsAnnotation?.let { existingPluginClasspaths?.removePrefix(it.value) }
            ?.split(pluginClasspathsAnnotation.delimiter)
            .orEmpty()
            .toMutableList()

        val friendPaths = friendPathsAnnotation?.let { existingFriendPaths?.removePrefix(it.value) }
            ?.split(friendPathsAnnotation.delimiter)
            .orEmpty()
            .toMutableList()
